    ...
    def run_episodes(self):
        ...
        for episode in range(len(self.mapnames_test)):
            ...
            if self.ep_idx % 4 == 0:
                ...
                conf_mat = confusion_matrix(self.pred_catnames, self.true_catnames, labels=self.include_classes)
                self.conf_mats.append(conf_mat)
                
                plt.figure(1)
                plt.clf()
                df_cm = pd.DataFrame(conf_mat, index = [i for i in self.include_classes],
                ...
                conf_mat = confusion_matrix(self.pred_catnames_all, self.true_catnames_all, labels=self.include_classes)
                plt.figure(1)
                plt.clf()
                df_cm = pd.DataFrame(conf_mat, index = [i for i in self.include_classes],
                  columns = [i for i in self.include_classes])
            ...
            for b in range(nbins):
                ...
                for s in range(spawns_per_bin):
                    ...
                    if mode=="train":
                        ...
                        self.true_ids.append(self.ithor_to_maskrcnn[obj['objectType']])
                        self.pred_catnames.append(match_nn_catname)

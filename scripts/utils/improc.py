...
class Summ_writer(object):
    ...
    def summ_oned(self, name, im, bev=False, logvis=False, maxval=0, max_along_y=False, norm=True, only_return=False):
        if self.save_this:
    ...
    def summ_lrtlist_bev(self, name, occ_memR, lrtlist, scorelist, tidlist, vox_util, lrt=None, already_mem=False, only_return=False, frame_id=None):
        if self.save_this:
